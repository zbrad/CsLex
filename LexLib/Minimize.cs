...
namespace LexLib
{
    public class Minimize
    {
        ...
        private void reduce()
        {
            ...
            set = new System.Collections.BitArray(dtrans_size);
            spec.anchor_array = new int[dtrans_size];
            spec.accept_list = new List<Accept>();
        ...
        {
            List<DTrans> new_list;
            int i;
            int size;
            List<DTrans> dtrans_group;
            DTrans first;
            int c;

            new_list = new List<DTrans>();
        ...
        private void minimize()
        {
            ...
            while (old_group_count != group_count)
            {
                ...
                for (i = 0; i < group_count; ++i)
                {
                    ...
                    new_group = new List<DTrans>();
                    added = false;

                    first = dtrans_group[0];
  ...
  pgroups();
        ...
        private void init_groups()
        {
            ...
            for (int i = 0; i < size; ++i)
            {
                ...
                if (!group_found)
                {
                    var dtrans_group = new List<DTrans>();
                    dtrans_group.Add(dtrans);
